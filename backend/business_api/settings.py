...
TENANT_CONFIG_PATH = os.path.join(BASE_DIR, 'tenants.yaml')
...
AUTHENTICATION_METHOD = 'BaseSecurity'

AUTHENTICATION_BACKENDS = ('BaseSecurity.backends.AuthenticateLogin', 
...
def ensure_database_options(db_config):
    """Убеждается, что в конфиге базы данных есть OPTIONS"""
...
def get_databases():
    ...
    databases = {
        'default': ensure_database_options(config['databases']['default'])
    }
    return databases
