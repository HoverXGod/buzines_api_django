...
@app.task(bind=True)
def clv_update(self, cls: type[str], db_name: type[str]):
...
@app.task(bind=True)
def sf_update(self, entry_id, db_name, **kwargs):
    with transaction.atomic(using, db_name):
...
@app.task(bind=True)
def pp_update(self, instance_id ,db_name , **kwargs):
    with transaction.atomic(using, db_name):
...
@app.task(bind=True)
def payment_analysis(self, payment_id, db_name):
    with transaction.atomic(using, db_name):
...
@app.task(bind=True)
def cohort_analysis(self, db_name):
    with transaction.atomic(using, db_name):
...
@app.task(bind=True)
def order_analysis(self, order_id, db_name):
    with transaction.atomic(using, db_name):
...
@app.task(bind=True)
def order_item_analysis(self, order_item_id, db_name, **extra_fields):
    with transaction.atomic(using, db_name):
